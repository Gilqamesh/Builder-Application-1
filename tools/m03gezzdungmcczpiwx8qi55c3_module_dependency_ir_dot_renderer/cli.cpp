...
int main(int argc, char** argv) {
    if (argc != 3) {
        std::cerr << std::format("usage: {} <target_module_name> <output_dot_path>", argv[0]) << std::endl;
        return 1;
    }

    const auto target_module_name = std::string_view(argv[1]);
    const auto output_dot_path = m03gagbhsnusi43zogoacgj2ez_filesystem::path_t(argv[2]);
    ...
    try {
        ...
        const auto output = m03gezzdungmcczpiwx8qi55c3_module_dependency_ir_dot_renderer::render(
            dependency_ir,
            target_module_name,
            output_dot_path
        );
