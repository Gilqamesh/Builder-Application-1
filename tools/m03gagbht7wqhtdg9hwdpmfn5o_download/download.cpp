...
static void validate_source_lock(const source_lock_t& source_lock) {
    if (source_lock.url.empty()) {
        throw std::runtime_error("m03gagbht7wqhtdg9hwdpmfn5o_download::fetch: source lock URL must not be empty");
    }

    if (source_lock.sha256.empty()) {
        throw std::runtime_error("m03gagbht7wqhtdg9hwdpmfn5o_download::fetch: source lock SHA-256 must not be empty");
    }
}
