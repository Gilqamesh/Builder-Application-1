...
extern "C" void phase__interface(const m03gagbhsujjf63n0w3r2w4q6h_build_phases::interface_phase_t* phase) {
    ...
    phase->install_interface(m03gagbhsnusi43zogoacgj2ez_filesystem::rooted_path_t(sources.root(), m03gagbhsnusi43zogoacgj2ez_filesystem::relative_path_t("tar.h")));
}
...
extern "C" void phase__library(const m03gagbhsujjf63n0w3r2w4q6h_build_phases::library_phase_t* phase) {
    ...
    const auto library = phase->build_library(
        { phase->build(sources.root() / m03gagbhsnusi43zogoacgj2ez_filesystem::relative_path_t("tar.cpp")) },
        {
            m03gagbhsmhr0naw0zpccv4gaq_cxx_toolchain::define_t("M03GAGBHTELDYU7PTBGNVOOTMB_TAR_TAR_PATH", "/usr/bin/tar")
        }
    );
