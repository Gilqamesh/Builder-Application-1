...
static m03gagbhsnusi43zogoacgj2ez_filesystem::path_t dot_path() {
    ...
    if (!m03gagbhsnusi43zogoacgj2ez_filesystem::exists(result) || !m03gagbhsnusi43zogoacgj2ez_filesystem::is_regular_file(result)) {
        throw std::runtime_error(std::format("m03gagbht6ja46uikb1ltan0x8_dot::render_svg: host tool '{}' does not exist or is not a regular file", result));
    }
...
m03gagbhsnusi43zogoacgj2ez_filesystem::path_t render_svg(
    ...
    if (output_svg_path.extension() != ".svg") {
        throw std::runtime_error(std::format("m03gagbht6ja46uikb1ltan0x8_dot::render_svg: output path '{}' must have .svg extension", output_svg_path));
    }

    if (!m03gagbhsnusi43zogoacgj2ez_filesystem::exists(dot_file) || !m03gagbhsnusi43zogoacgj2ez_filesystem::is_regular_file(dot_file)) {
        throw std::runtime_error(std::format("m03gagbht6ja46uikb1ltan0x8_dot::render_svg: DOT file '{}' does not exist or is not a regular file", dot_file));
    }

    if (m03gagbhsnusi43zogoacgj2ez_filesystem::exists(output_svg_path)) {
        throw std::runtime_error(std::format("m03gagbht6ja46uikb1ltan0x8_dot::render_svg: output path '{}' already exists", output_svg_path));
    }
    ...
    try {
        ...
        if (!m03gagbhsnusi43zogoacgj2ez_filesystem::exists(output_svg_path)) {
            throw std::runtime_error(std::format("m03gagbht6ja46uikb1ltan0x8_dot::render_svg: expected output '{}' to exist but it does not", output_svg_path));
        }
    } catch (...) {
