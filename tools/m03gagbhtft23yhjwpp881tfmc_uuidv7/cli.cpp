...
int main(int argc, char** argv) {
    if (argc == 1) {
        try {
            std::cout << m03gagbht2l61mj6qitacwbmea_base36::encode(m03gagbhtft23yhjwpp881tfmc_uuidv7::uuidv7::generate().bytes()) << std::endl;
    ...
    try {
        ...
        if (option == "--base36") {
            ...
            std::cout << uuid_string(m03gagbhtft23yhjwpp881tfmc_uuidv7::uuidv7::generate().bytes()) << std::endl;
        } else {
            std::cerr << "usage: " << argv[0] << " [--base36|--uuid]\n";
            return 1;
        }
